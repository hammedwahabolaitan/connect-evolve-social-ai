...
const Sidebar = () => {
  const menuItems = [
    { icon: Users, label: 'Friends', count: '1.2k' },
    { icon: Calendar, label: 'Events', count: '3' },
    { icon: Image, label: 'Photos', count: '247' },
    { icon: Video, label: 'Videos', count: '12' },
    { icon: MessageCircle, label: 'Groups', count: '8' },
    { icon: Heart, label: 'Saved', count: '45' },
  ];
      ...
      <Card className="p-4">
        ...
        <div className="space-y-3">
          {['#AI Technology', '#Social Media', '#Tech Innovation', '#Future Tech'].map((tag, index) => (
            <div key={index} className="text-blue-600 hover:text-blue-700 cursor-pointer text-sm font-medium">
              {tag}
            </div>
          ))}
        </div>
